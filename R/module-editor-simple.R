...
mod_editor_simple_server <- function(input, output, session, initial_code,
  ...
  ## Will only run once:
  shiny::observe({
    update_ace_editor(session, "editor", value = initial_code)
  })
  ...
  shiny::observe({
    update_ace_editor(session, "editor", border = editor_border(rv$validation))
  })
  ...
  shiny::observeEvent(
    input$uploaded_file, {
      if (!is.null(input$uploaded_file)) {
        ...
        update_ace_editor(session, "editor", value = code)
  ...
  set_state <- function(state) {
    ...
    update_ace_editor(session, "editor", value = state$editor)
  }
