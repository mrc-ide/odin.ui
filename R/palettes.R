odin_ui_palettes <- function(palette = NULL) {
  out <- list(
    ...
    funky = grDevices::colorRampPalette(
      c("#A6CEE3", "#438EC0", "#63A8A0", "#98D277", "#3BA432", "#B89B74", 
  ...
  if (!is.null(palette)) {
    if (!palette %in% names(out)) {
      stop(sprintf("palette '%s' is not available", palette))
    }
    out <- out[[palette]]
  }
